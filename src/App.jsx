    ...
    {showBanner&&<div style={{marginBottom:14,borderRadius:10,overflow:"hidden",border:`1px solid ${isFirstOfMonth?"#5DCAA5":daysLeft===0?"#F09595":"#FAC775"}`}}>
      <div style={{background:isFirstOfMonth?"#E1F5EE":daysLeft===0?"#FCEBEB":"#FFF9EE",padding:"11px 16px",display:"flex",justifyContent:"space-between",alignItems:"flex-start",gap:12,flexWrap:"wrap"}}>
        ...
        <div style={{display:"flex",gap:8,alignItems:"center",flexShrink:0,flexWrap:"wrap"}}>
          {isFirstOfMonth
            ?<Btn small onClick={()=>setView("monthly")}>Download {prevMonth} statement</Btn>
            :<><Btn small onClick={()=>setView("monthly")}>View stock statement</Btn><Btn small variant="ghost" onClick={()=>setView("settings")}>Lock closing stock</Btn></>
          }
    ...
    <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:14}}>
      ...
      <div style={{display:"flex",flexDirection:"column",gap:12}}>
        ...
        <Card>
          ...
          {[{icon:"🎂",label:"New production entry",view:"production",roles:["owner","production"]},{icon:"🧾",label:"Scan purchase receipt",view:"receipts",roles:["owner","production"]},{icon:"💸",label:"Log cash expense",view:"expenses",roles:["owner"]},{icon:"📊",label:"Monthly P&L report",view:"monthly",roles:["owner"]},{icon:"📄",label:"Create invoice",view:"invoices",roles:["owner","customer_service"]},{icon:"🛒",label:"Shopping list",view:"shopping",roles:["owner","production"]}].filter(a=>a.roles.includes(user?.role)).map(a=><div key={a.view} onClick={()=>setView(a.view)} style={{display:"flex",alignItems:"center",gap:10,padding:"7px 8px",borderRadius:8,cursor:"pointer",marginBottom:2}} onMouseEnter={e=>e.currentTarget.style.background="#F0E9DB"} onMouseLeave={e=>e.currentTarget.style.background="transparent"}>
...
function MonthlyOverview({inventory,productions,expenses,company}){
  ...
  const cur=new Date().toISOString().slice(0,7)
  const [sel,setSel]=useState(months[0]||cur)
  ...
  const rev=paid.reduce((s,p)=>s+(p.salePrice||0),0)
  const prodCost=mProds.reduce((s,p)=>s+(p.cost||0)+(p.deliveryCost||0),0)
  const mExp=expenses.filter(e=>e.date?.startsWith(sel)&&e.category!=="Ingredients")
  ...
  const dl=()=>{
    ...
    h1{font-size:22px;font-weight:700;color:${company?.primaryColor||"#C8912A"}}
    ...
    .sec{margin-top:24px;font-size:14px;font-weight:700;color:#291608;border-bottom:2px solid #C8912A;padding-bottom:4px;margin-bottom:10px}
    </style></head><body>
    ...
    <h2>${monthLabel}</h2>
    <div class="grid">
      <div class="card"><div class="label">Revenue</div><div class="val">₦${Math.round(rev).toLocaleString()}</div></div>
      ...
      <div class="card"><div class="label">Net profit</div><div class="val" style="color:${profit>=0?"#357A52":"#B03A2E"}">₦${Math.round(profit).toLocaleString()}</div></div>
    </div>
  ...
  return <div>
    ...
    <div style={{display:"flex",alignItems:"center",gap:12,marginBottom:18,flexWrap:"wrap"}}>
      <select value={sel} onChange={e=>setSel(e.target.value)} style={{padding:"7px 12px",borderRadius:8,border:"1px solid var(--border)",background:"var(--panel)",fontSize:13,color:"var(--text)"}}>
        {(months.length?months:[cur]).map(m=><option key={m} value={m}>{new Date(m+"-02").toLocaleDateString("en-NG",{month:"long",year:"numeric"})}</option>)}
      </select>
    ...
    <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(130px,1fr))",gap:10,marginBottom:16}}>
      {[
        {l:"Revenue",v:fmt(rev),s:`${paid.length} paid orders`,c:"var(--gold)"},
