    ...
    {user?.role==="owner"&&<Card style={{marginBottom:14,padding:"14px 18px"}}>
      <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:12,flexWrap:"wrap",gap:8}}>
        ...
        <div style={{display:"flex",gap:12}}>
          {[["#C8912A","Revenue"],["#378ADD","Cost"],["#1D9E75","Profit"]].map(([c,l])=><div key={l} style={{display:"flex",alignItems:"center",gap:4}}><div style={{width:10,height:10,borderRadius:2,background:c}}/><span style={{fontSize:11,color:"var(--muted)"}}>{l}</span></div>)}
        </div>
      </div>
      <div style={{position:"relative",height:chartH+"px",marginBottom:4}}>
        ...
        <div style={{display:"flex",gap:6,height:"100%",alignItems:"flex-end",paddingRight:36}}>
          {weeks.map((w,wi)=><div key={wi} onClick={()=>setActiveWeek(activeWeek===wi?null:wi)} style={{flex:1,display:"flex",gap:2,alignItems:"flex-end",height:"100%",cursor:"pointer"}}>
            {[[w.rev,"#C8912A"],[w.cost,"#378ADD"],[w.profit,"#1D9E75"]].map(([val,col],bi)=><div key={bi} style={{flex:1,height:Math.round((val/maxVal)*chartH)+"px",background:col,borderRadius:"3px 3px 0 0",opacity:activeWeek===wi?1:0.85,transition:"opacity 0.15s"}}/>)}
          </div>)}
        </div>
      </div>
